            ...
            <form method="POST" action="{{ route('sceme.store') }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-sm-6 mb-3">
                        ...
                        <input name="sceme_name" id="sceme_name" type="text" value="{{ old('sceme_name')}}"
                            class="form-control @error('sceme_name') is-invalid @enderror" required>
                        @error('sceme_name')
                        <small class="text-danger">{{ 'maksimal karakter 255' }}</small><br>
                        @enderror

                        <label for="sceme_code" class="col-form-label">Kode Skema</label>
                        <input id="sceme_code" name="sceme_code" type="text" value="{{ old('sceme_code')}}"
                            class="form-control @error('sceme_code') is-invalid @enderror" required>
                        @error('sceme_code')
                        <small class="text-danger">{{ 'maksimal karakter 255' }}</small><br>
                        @enderror
                        ...
                        <select id="sceme_status" name="sceme_status" class="form-select"
                            ...
                            <option value="2" {{ old('sceme_status')==2 ? 'selected' :''}}>Tidak Aktif</option>
                            <option value="1" {{ old('sceme_status')==1 ? 'selected' :''}}>Active</option>
                        </select>
                        ...
                        <select id="sceme_bnsp" name="sceme_bnsp" class="form-select"
                            aria-label="Default select example" required>
                            <option disabled>Pilih Status</option>
                            <option value="1" {{ old('sceme_bnsp')==1 ? 'selected' :''}}>Active</option>
                            <option value="2" {{ old('sceme_bnsp')==2 ? 'selected' :''}}>Tidak Aktif</option>
                            <option value="3" {{ old('sceme_bnsp')==3 ? 'selected' :''}}>Pengajuan</option>
                        </select>

                    </div>
                    <div class="col-sm-6">
                        <div class="row mb-3">
                            <label for="sceme_file" class="col-sm-4 col-form-label">Tambah File <i>Pdf</i></label>
                            <div class="col-sm-8">
                ...
                <div class="row mb-3">
                    <label for="sceme_detail" class="col-form-label">Deskripsi Skema</label><br>
                    ...
                    <div class="col-sm-12">
                        <input id="sceme_detail" type="hidden" value="{{ old('sceme_detail') }}" name="sceme_detail"
                            required>
